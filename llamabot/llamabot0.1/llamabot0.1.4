...
async def 도움말(ctx):
    ...
    embed.add_field(name="`라마 핑`", value="웹소켓 딜레이 출력", inline=True)
    embed.add_field(name="`라마 안녕`", value="라마봇과 인사", inline=True)
    embed.add_field(name="`라마 날씨`", value="현재 날씨 출력", inline=True)
    embed.add_field(name="`라마 개소리`", value="랜덤 개소리 출력", inline=True)
    embed.add_field(name="`라마 채팅 [메세지]`", value="[메세지] 라마봇이 출력", inline=True)
    
    embed.set_footer(text="라마봇0.1.4   개발자 : @eukkcha")
...
async def 날씨(ctx):
    ...
    message = f"남조선괴뢰통신정보국포착감시프로세스 가동 중\n:llama:`{temp}({description}), 미세먼지 {dust}`:llama:"
...
async def 핑(ctx):
    latancy = bot.latency
    await ctx.send(f'라마국 긴급공멸프로토콜 속도 `{round(latancy * 1000)} ms`')
